...
export default function ArticleVenteModal({ 
  ...
  useEffect(() => {
    ...
    // Si le montant encaissé n'a pas été modifié manuellement, le mettre à jour
    if (mtEncaisse === 0 || mtEncaisse === totalLigne) {
      setMtEncaisse(total);
    }
      ...
      <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg border border-gray-200">
        ...
        <div className="mb-4">
          ...
          <Input
            type="number"
            value={mtEncaisse}
            onChange={(e) => setMtEncaisse(parseFloat(e.target.value))}
